...
data_load_adaptation<-function(file_name){
  file_path=paste("raw_data/",file_name,sep="")
  
  data = list.files(path = file_path, full.names = T) %>%
    map(read.csv, col.names=c("time","ID","mode","trial_num","stimu_pos","delay","sound_end_time","trigger_time","knife_contact_time","time_diff","arm_delay","rating_pain","rating_unp",
    ...
    mutate(time=as.character(time), ID=factor(ID),mode=factor(mode),stimu_pos=factor(stimu_pos),trial_num=as.integer(trial_num),
           ...
           time_diff=as.numeric(time_diff),arm_delay=as.numeric(arm_delay),rating_pain=as.numeric(rating_pain),rating_unp=as.numeric(rating_unp),
  ...
  assign("data_adapt",data,env=.GlobalEnv)
