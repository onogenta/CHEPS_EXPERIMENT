...
p_con_1<-ggplot(data=data_pos_simul,aes(x=simultaneity,y=unp_avg,fill=stimu_pos))
...
p_con_4<-p_con_3+
  geom_jitter(
    data=data_con,
    aes(x=simultaneity,y=rating_unp,shape=stimu_pos),
    stat = "identity",
    size=2,
...
p_con_5<-p_con_4+
  geom_errorbar(
    data=data_pos_simul,
    aes(x=simultaneity,ymin=unp_avg-unp_sd,ymax=unp_avg+unp_sd),
    position = position_dodge(width=1),
    width=.2
  )
  

ggsave(file = "unp_con.png", plot = p_con_5, dpi = 100, width = 8.27,height = 11.69)