...
data_load_vanish<-function(file_name){
  file_path=paste("raw_data/",file_name,sep="")
  
  data = list.files(path = file_path, full.names = T) %>%
    map(read.csv, col.names=c("time","ID","mode","trial_num","stimu_pos","delay","sound_end_time","trigger_time","knife_contact_time","time_diff","arm_delay","vanish","rating_pain","rating_unp",
                              "simultaneity","Q1","Q2","Q3","Q4","Q5"),colClasses = "character", header = F, sep = ",",na.strings="na")%>%
    reduce(rbind)%>%
    mutate(time=as.character(time), ID=factor(ID),mode=factor(mode),stimu_pos=factor(stimu_pos),trial_num=as.integer(trial_num),
           ...
           time_diff=as.numeric(time_diff),arm_delay=as.numeric(arm_delay),vanish=factor(vanish),rating_pain=as.numeric(rating_pain),rating_unp=as.numeric(rating_unp),
    ...
    mutate(bin=cut(delay,breaks=c(-10000,-801,750,10000)*0.001,right = FALSE,
                   labels = c("-1200~-800","-200~200","800~1200")))
  ...
  assign("data_vanish",data,env=.GlobalEnv)
