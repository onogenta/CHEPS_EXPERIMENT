data_load_vanish("all_vanish")
file_pass=paste("result/vanish_all/","/",sep="")

data_vanish<-data_vanish%>%
  filter(mode=="test_vanish")%>%
  group_by(ID)%>%
  mutate(pain_cwc=rating_pain-mean(rating_pain),unp_cwc=rating_unp-mean(rating_unp),Q1_cwc=Q1-mean(Q1),Q2_cwc=Q2-mean(Q2),Q3_cwc=Q3-mean(Q3))%>%
  ungroup()


data_sum<-data_vanish%>%
  filter(mode=="test_vanish")%>%
  group_by(bin,ID,vanish)%>%
  summarise(pain_cwc_avg=mean(pain_cwc),unp_cwc_avg=mean(unp_cwc),Q1_cwc_avg=mean(Q1_cwc),Q2_cwc_avg=mean(Q2_cwc),Q3_cwc_avg=mean(Q3_cwc))%>%
  ungroup()

data_sum_prob<-data_vanish%>%
  filter(mode=="test_vanish")%>%
  group_by(bin,vanish)%>%
...
p_1<-ggplot(data=data_sum,aes(x=bin,y=pain_cwc_avg,fill=vanish))
...
p_3<-p_2+
  geom_boxplot(width=0.8,outlier.colour = "red",outlier.size = 1,outlier.shape = 8,position = position_dodge(1))+
...
p_pain<-p_3+
  geom_line(aes(x=as.numeric(bin),y=simul_prob*scaler,colour=vanish),size=4,data = data_sum_prob)+
  # geom_point(aes(x=Delay(ms),y=simul_prob*scaler),col="black")+
  template

file_name=paste(file_pass,"plot_bin_pain_all.png",sep="")
...
p_1<-ggplot(data=data_sum,aes(x=bin,y=unp_cwc_avg,fill=vanish))

p_2<-p_1+
  labs(x="Delay(ms)",y="unp(cwc)",title="unp")+
...
file_name=paste(file_pass,"plot_bin_unp_all.png",sep="")
...
p_1<-ggplot(data=data_sum,aes(x=bin,y=Q1_cwc_avg,fill=vanish))

p_2<-p_1+
  labs(x="Delay(ms)",y="Q1(cwc)",title="Q1")+
...
file_name=paste(file_pass,"plot_bin_Q1_all.png",sep="")
...
p_1<-ggplot(data=data_sum,aes(x=bin,y=Q2_cwc_avg,fill=vanish))

p_2<-p_1+
  labs(x="Delay(ms)",y="Q2(cwc)",title="Q2")+
...
file_name=paste(file_pass,"plot_bin_Q2_all.png",sep="")
...
p_1<-ggplot(data=data_sum,aes(x=bin,y=Q3_cwc_avg,fill=vanish))

p_2<-p_1+
  labs(x="Delay(ms)",y="Q3(cwc)",title="Q3")+
...
file_name=paste(file_pass,"plot_bin_Q3_all.png",sep="")
