...
data_pos_1$stimu_pos<- factor(data_pos_1$stimu_pos,levels = c("wrist","upper_arm"))
data_pos_2$stimu_pos<- factor(data_pos_2$stimu_pos,levels = c("wrist","upper_arm"))
...
p_1<-ggplot(data=data_sum,aes(x=delay,y=pain_avg,fill=stimu_pos))
...
file_name=paste(file_pass,data$ID[1],"pain_all.png",sep="")
