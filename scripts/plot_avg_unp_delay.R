...
data_diff<-data_avg%>%
  group_by(ID)%>%
  mutate(pain_diff=pain_avg/pain_avg[delay==0&stimu_pos==1],unp_diff=unp_avg/unp_avg[delay==0&stimu_pos==1],Q1_diff=Q1_avg/Q1_avg[delay==0&stimu_pos==1])%>%
  ungroup()
...
data_diff_sum<-data_diff%>%
  ...
  summarise(pain_avg=mean(pain_diff),simul_avg=mean(simul_avg),unp_avg=mean(unp_diff),Q1_avg=mean(Q1_diff),pain_sd=sd(pain_diff),unp_sd=sd(unp_diff),Q1_sd=sd(Q1_diff))
...
p_3<-p_2+
  ...
  # geom_jitter(
  #   data=data_all,
  #   aes(x=delay,y=rating_unp,shape=stimu_pos),
  #   stat = "identity",
  #   size=1.5,
...
p_pos<-ggplot(data = data_diff_sum_pos,aes(x=stimu_pos,y=unp_avg,fill=stimu_pos))+
  theme_classic()+
  geom_bar(stat = "identity",width=0.5,position = position_dodge(0.1))+
  geom_errorbar(aes(ymin=unp_avg-unp_sd,ymax=unp_avg+unp_sd), position = position_dodge(width=0.1),
                width=.1)+
...
ggsave(file="unp_pos.png",plot = p_pos,dpi=100,width = 16,height = 22)
